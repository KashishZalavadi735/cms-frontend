...
export const getAllProfessor = async (page: number = 1, limit: number = 5, search: string = "") => {
...
export const updateProfessor = async (id:number, ProfessorData: any) => {
   const response = await axios.put(ADMIN_API.UPDATE_PROFESSOR(id), ProfessorData, {
        headers: getAuthHeader()
    });
    return response.data.data; 
};
...
export const updateProfessorSubjects = async (id:number, data: {subjectIds:number[]}) => {
    const response = await axios.put(ADMIN_API.UPDATE_PROFESSOR_SUBJECTS(id), data, {
        headers: getAuthHeader()
    });
    return response.data.data;
};
